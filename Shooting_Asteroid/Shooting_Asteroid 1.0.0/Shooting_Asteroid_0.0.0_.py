import random
import pygame, tank, enemy_asteroid
pygame.init()
...
while running:
    ...
    for enemy_asteroids in asteroids_array[:]:
        enemy_asteroids[1] += speed_asteroids 
        enemy_asteroid.asteroids(screen, (80, 80, 80), enemy_asteroids[0], enemy_asteroids[1], 20)

        # Check if asteroid has gone off screen
        if enemy_asteroids[1] >= 620:
            asteroids_array.remove(enemy_asteroids)

        # Collision detection
        for bullet in bullet_array[:]:
            if (
                enemy_asteroids[0] - 20 <= bullet[0] + 35 <= enemy_asteroids[0] + 20 and  # Check X-axis overlap
                enemy_asteroids[1] - 20 <= bullet[1] - 20 <= enemy_asteroids[1] + 20     # Check Y-axis overlap
               ):
                asteroids_array.remove(enemy_asteroids)
                bullet_array.remove(bullet)
